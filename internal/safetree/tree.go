// Package safetree builds and verifies a private checksummed file tree. It has
// no artifact IDs, publication policy, retention, or backup format knowledge.
package safetree
...
// NewBuilder writes inside a caller-owned, private directory. The caller owns
// its creation, failure cleanup and eventual publication. A builder is single-use.
func NewBuilder(ctx context.Context, dir string, limits Limits) (*Builder, error) {
	files := filepath.Join(dir, "files")
	if err := os.Mkdir(files, 0755); err != nil {
		return nil, err
	}
	return &Builder{ctx: ctx, stage: dir, files: files, limits: effective(limits), records: map[string]record{}}, nil
}
...
// ExtractTarGzip writes only regular files and directories, then seals the tree.
func (b *Builder) ExtractTarGzip(r io.Reader) (Info, error) {
	ctx := b.ctx
...
// ImportRoot copies an already-opened source without sharing mutable files.
func (b *Builder) ImportRoot(source *os.Root) (Info, error) {
	ctx := b.ctx
...
// Verify checks the exact manifest entry set and all file checksums.
func Verify(ctx context.Context, dir string) (Info, error) {
	if err := ctx.Err(); err != nil {
		return Info{}, err
	}
