...
Audit records are stored in SQLite alongside the managed state. Each authenticated management operation first commits a `pending` record; an unavailable audit store returns 503 before the operation starts. Completion records the actual HTTP status and `success` or `failure`. A failed domain verification or partially failed cleanup is a failure even when its API returns 200. A crash or a failed final audit write leaves `pending`: inspect the target's current state before retrying. Audit completion is not a distributed transaction with the operation, so pending does not imply that no change happened.
